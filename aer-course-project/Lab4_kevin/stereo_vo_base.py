...
class VisualOdometry:
    ...
    def pose_estimation(self, features_coor):
        ...
        ransac.fit(f_r_prev, f_r_cur)

        # Get the inlier mask; this is a boolean mask of the same size as the input data
        # True for inliers, False for outliers
        inlier_mask = ransac.inlier_mask_

        # Use the inlier mask to filter the input data
        f_r_prev_filtered = f_r_prev[inlier_mask]
        f_r_cur_filtered = f_r_cur[inlier_mask]

        # replace (1) the dummy C and r to the estimated C and r. 
        #         (2) the original features to the filtered features
        return C, r, f_r_prev_filtered, f_r_cur_filtered
