...
class GeoController():
    ...
    def _compute_desired_force_and_euler(self,
        ...
        body_acc = -Kp * pos_e - Kv * vel_e

        #NOTE: p refers to pybullet lib
        #R_bw = np.array(p.getMatrixFromQuaternion(cur_quat)).reshape(3, 3)
        #grav = np.matmul(R_bw,np.array([0,0,self.grav]))
        print(cur_quat)
        drn = cur_quat[-1]
        #print(drn)
        grav = np.array([0,0,drn * self.grav])
        des_acc = body_acc + grav+target_acc

        #---------Tip 2: Compute the desired thrust command--------#
        norm_acc = np.linalg.norm(des_acc)
        desired_thrust = self.mass * norm_acc


        #---------Tip 3: Compute the desired attitude command--------#
        X_c = np.array([np.cos(reference_yaw),np.sin(reference_yaw),0])
        Y_c = np.array([-np.sin(reference_yaw),np.cos(reference_yaw),0])
        # breakpoint()
        z_B = des_acc/norm_acc
