...
class VisualOdometry:
    ...
    def pose_estimation(self, features_coor):
        ...
        w_total = len(cur_point_cloud) #w_j is 1

        centroid_prev_total = np.mean(prev_point_cloud,axis=0)
        centroid_cur_total = np.mean(cur_point_cloud,axis=0)

        W_ba = []

        for idx in range(len(cur_point_cloud)):
            cur_mat = cur_point_cloud[idx] - centroid_cur_total
            prev_mat = prev_point_cloud[idx] - centroid_prev_total

            sum_mat = cur_mat @ np.transpose(prev_mat)

            W_ba = W_ba + sum_mat
        
        W_ba = W_ba / w_total

        V,scalar,U_transpose = np.linalg.svd(W_ba)

        prod = np.linalg.det(np.transpose(U_transpose)) * np.linalg.det(V)
        
        C = V @ np.array([[1,0,0],[0,1,0],[0,0,prod]])@U_transpose

        r = -(np.transpose(C)@ centroid_cur_total) + centroid_prev_total
